...
func (c *JimyagCustom) Filter(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeInfo *framework.NodeInfo) *framework.Status {
	...
	klog.Infof("Filter %s/%s: nodeName: %s", pod.Namespace, pod.Name, nodeInfo.Node().Name)
	data, err := state.Read(StateKye)
	// 如果没有拿到 state，直接返回
	if err != nil {
		return framework.NewStatus(framework.Error, err.Error())
	}
	klog.Infof("Filter %s/%s: stateData: %s", pod.Namespace, pod.Name, data.(*StateData).Msg)
